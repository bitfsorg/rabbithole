...
Bitcoin 交易结构与合约脚本是系统设计主线：

- 交易结构：每个核心能力都需映射到明确的交易模板、输入输出约束与 UTXO 状态迁移。
- 合约脚本：每条支付与权限关键路径都需映射到可验证的 Script 条件（锁定条件、解锁条件、失败回退路径）。
- 审查顺序建议：先看“第四章 Metanet 交易格式”与“第十一章 买卖交易 -- HTLC 原子交换”，再看 Rabin/CLTV/收益权等合约扩展。
...
**存储决策矩阵** — 选择链上 (DataTx) 还是链下 (Daemon/LFCP):

| 维度 | 链上 DataTx (OP_DROP) | 链下 Daemon (LFCP) |
|------|----------------------|-------------------|
| 适用大小 | < 100KB (单笔交易上限约 10MB, 但成本随大小线性增长) | 不限 |
| 持久性 | 永久不可删, 矿工全节点存储 | Owner 控制, 可删可改 |
| 可用性 | 只要 BSV 网络存在即可获取 | 依赖 daemon 在线或 CDN 节点缓存 |
| 成本模型 | 按字节一次性付费 (~0.5 sat/byte), 永不再付 | 存储+带宽持续成本 (自建服务器或 CDN 托管费) |
| 访问速度 | 需查链获取交易, 延迟较高 | HTTP 直接获取, 低延迟 |
| 隐私 | 加密内容永久上链, 未来密码学突破有理论风险 | 服务器控制, 可随时删除 |
| 典型场景 | 关键证书/合同/代码哈希/法律存证/数字遗产 | 日常文件/媒体/频繁更新的内容/大文件 |

**默认推荐**: 链下 (LFCP)。仅在需要永久不可变存证时选择链上。`bitfs put --onchain` 显式触发链上存储。
...
| 支付 | HTLC 购买结算 | 下载计费发票控制 | 托管结算与节点间结算 |
