...
- [ ] **Agent Friendly 落地** — b* `--json` 缺 JSON Schema、无 SDK 层、x402 多轮交互、无 MCP 适配。路径: JSON Schema → libbitfs-ts → x402 单步 API → MCP server
...
- [ ] **存储决策矩阵** — 链上 DataTx vs 链下 LFCP 的选择标准，补充到设计文档

## 客户端生态

按优先级排序，等协议定型后依次启动:

1. **Web Dashboard** — React 19 + Vite + TailwindCSS，Go embed 嵌入 daemon `/_dashboard/*`
