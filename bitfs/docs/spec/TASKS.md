...
### 任务 1：libbitfs-go/method42 -- Method 42 ECDH 加密引擎
- **包**：`libbitfs-go/method42/`
...
### 任务 2：libbitfs-go/wallet -- HD 钱包（BIP32/BIP39）
- **包**：`libbitfs-go/wallet/`
...
- **验收标准**：
  ...
  - [x] 资金不足错误信息清晰
- **估计测试数**：40
- **依赖**：go-sdk（transaction、script、ec）、libbitfs-go/wallet

---

## 第二阶段：文件系统（DAG + 验证 + 存储）

### 任务 4：libbitfs-go/metanet -- Metanet DAG 解析器
- **包**：`libbitfs-go/metanet/`
...
- **估计测试数**：65
- **依赖**：libbitfs-go/tx、go-sdk（ec）

### 任务 5：libbitfs-go/spv -- SPV 轻客户端
- **包**：`libbitfs-go/spv/`
...
### 任务 6：libbitfs-go/storage -- 内容存储
- **包**：`libbitfs-go/storage/`
...
### 任务 7：libbitfs-go/paymail -- Paymail 身份解析
- **包**：`libbitfs-go/paymail/`
...
### 任务 8：libbitfs-go/x402 -- x402 支付协议
- **包**：`libbitfs-go/x402/`
...
- **估计测试数**：30
- **依赖**：go-sdk（transaction、script）、libbitfs-go/method42
...
- **验收标准**：
  ...
  - [x] 免费内容自动解密
- **估计测试数**：75
- **依赖**：cobra、libbitfs-go/paymail、libbitfs-go/method42、libbitfs-go/x402
