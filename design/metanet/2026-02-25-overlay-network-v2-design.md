...
- **职责**: 收集 ON 交易，构建 ML Block，执行 SHA256 POW
- **收入**: MNT 出块奖励 (Coinbase) + ON 交易手续费
- **前提条件**: 无。任何拥有算力的节点均可参与出块竞争
- **设计原则**: 挖矿与 CDN 服务完全解耦——矿工负责 MNT 发行，Storage Provider 负责数据存储，两者通过 MNT 市场连接
...
```
算法:          SHA256d (与 Bitcoin 完全一致)
目标出块时间:   ~5 分钟 (Bitcoin 的一半)
难度调整:       每 2016 个 ML Block (约 1 周)
初始难度:       较低 (允许 CPU 挖矿)
不合并挖矿:     独立 SHA256 POW，不与 BSV 合并挖矿
```

**"Bitcoin at 2× Speed" 设计**：所有结构参数（50 MNT 奖励、210,000 块减半、21M 总量、2016 块难度调整）与 Bitcoin 完全一致，仅出块时间缩短为 5 分钟。同一套参数，双倍心跳。

5 分钟出块适合 CDN overlay：
- 存储证明更快确认
- MNT 支付更快结算
- 难度调整更灵敏（1 周 vs Bitcoin 的 2 周，对早期算力波动响应更快）
- 孤块率不是问题——ML Block 通过 BSV mempool 传播（秒级）

由于 BSV 提供最终性，POW 的目的仅是公平分配出块权和 MNT 发行，不需要 BSV 级别的算力来保护链安全。出块奖励完全由 POW 矿工竞争取得，不与 x402 检索收入挂钩。
...
总量:          21,000,000 MNT          ← 与 Bitcoin 完全相同
初始出块奖励:   50 MNT / ML Block       ← 与 Bitcoin 完全相同
减半间隔:      210,000 ML Blocks       ← 与 Bitcoin 完全相同
日历减半周期:   约 2 年 (5 分钟出块)     ← Bitcoin 的一半 (Bitcoin 为 4 年)
最小单位:      1 satoshi = 0.00000001 MNT
```

### 5.1.1 发行时间表

| Epoch | 年份 | 奖励/Block | 年铸造量 | 累计发行 | 占总量 |
|-------|------|-----------|---------|---------|--------|
| 0 | Y0-2 | 50 MNT | 5,256,000 | 10,500,000 | 50.0% |
| 1 | Y2-4 | 25 MNT | 2,628,000 | 15,750,000 | 75.0% |
| 2 | Y4-6 | 12.5 MNT | 1,314,000 | 18,375,000 | 87.5% |
| 3 | Y6-8 | 6.25 MNT | 657,000 | 19,687,500 | 93.75% |
| 4 | Y8-10 | 3.125 MNT | 328,500 | 20,343,750 | 96.875% |

**2 年减半的经济依据**：

全球 CDN 市场 ~$25-30B/年（2025），年增长 ~15-20%。目标 10,000 个专业节点（年运营成本 ~$10K/节点）。网络增长模型：

| 阶段 | 时间 | 节点数 | 有机年收入 | 状态 |
|------|------|--------|-----------|------|
| 引导期 | Y0-2 | 50-500 | $0-5M | 完全依赖出块奖励 |
| 早期增长 | Y2-4 | 500-2,000 | $5-50M | 首批存储合约 |
| 增长期 | Y4-6 | 2,000-5,000 | $50-200M | 有机收入开始超过奖励 |
| 规模期 | Y6-8 | 5,000-10,000 | $200M-1B | 自我维持 |

关键交叉点在 Y4-6：有机费用收入超过出块奖励，恰好对应 87.5% 供应已发行。1 年减半太激进（Y3 即枯竭），4 年太慢（CDN 市场等不起）。2 年减半在激励早期建设者和保证长期可持续之间取得最佳平衡。
...
- 挖矿与存储完全解耦——矿工赚 Coinbase，SP 赚合约付款，**没有"存得多就挖得多"的漏洞**
- Filecoin 的造假根源是"存储量 = 出块权"；我们没有这种绑定，从根本上消除了造假激励
