...
\begin{lstlisting}
 ...
 |  x402 downloads, metadata permanence                    |
 |  Currency: BSV                                          |
 +--------------------------------------------------------+
...
The responsibilities of the two chains are clearly divided. The BSV main chain records file ownership (the Metanet DAG), handles purchase and sale transactions (HTLC atomic swaps and x402 payments), and stores metadata permanently. It faces end users and AI agents. The Metanet Chain records archive contracts, storage proofs, payment channel settlements, and mining rewards. It faces content owners and node operators. Data never needs to cross between the two chains; they serve complementary roles in the same ecosystem.
...
BSV is the user-facing currency. When someone downloads a file through the Metanet Network, they pay the x402 retrieval fee in BSV. When someone purchases access to encrypted content through an HTLC, they pay in BSV. Regular users of BitFS never need to acquire, hold, or even know about the MNT token.
...
Metanet Nodes have three revenue streams: x402 retrieval fees paid in BSV by users downloading content, archive contract fees paid in MNT by content owners ensuring data availability, and mining rewards paid in MNT by the protocol for producing blocks.
...
Retrieval incentive & Weak (no direct reward) & Strong (x402 revenue) \\
...
The Metanet Network creates a natural marketplace between content owners and node operators. When a Metanet Node serves a file and collects the x402 retrieval fee, the question is immediate: who keeps the revenue?

Content owners set a \texttt{revenue\_share} parameter in file metadata, specifying what percentage of x402 fees the serving node retains and what percentage flows back to the owner. A typical split might allocate 70\% to the node and 30\% to the owner, though the ratio is entirely at the discretion of the owner.
...
While individual x402 payments work well for discrete file downloads, streaming media and large file transfers require a more efficient payment mechanism. Paying a separate on-chain transaction for every kilobyte of a video stream would be impractical even with the low fees of BSV.
...
\midrule
Primary incentive & Retrieval (x402 fees) & Storage (PoSt rewards) & None & One-time storage fee & Service contracts \\
\addlinespace
...
\addlinespace
User payment & BSV (x402 micropayments) & FIL & Free & Free after upload & Subscription / bandwidth \\
\addlinespace
...
A Metanet Node is fundamentally a BitFS daemon running in provider mode. It serves encrypted content over HTTP, processes x402 payments, maintains archive contracts, submits storage proofs, and optionally participates in merge-mining. The \texttt{metanet} CLI wraps these capabilities with network-specific configuration and monitoring.
...
\item C.~S.~Wright, ``An Immutable File and Data Store,'' nChain, 2025. Method~42: Deterministic key derivation for per-file encryption using ECDSA/secp256k1.

\item S.~Nakamoto, ``Bitcoin: A Peer-to-Peer Electronic Cash System,'' 2008.

\item GB2608179A, ``Multi-level Blockchain,'' UK Intellectual Property Office, 2025. Embedding secondary data chains on a core blockchain.
