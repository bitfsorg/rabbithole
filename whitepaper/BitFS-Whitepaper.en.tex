...
|  +----------+ +----------+ +---------+ +------------------+  |
|  | Method 42| | SPV      | | Rabin   | | Revenue Share /  |  |
...
A filename exists only in the child entry list of the parent directory, not in the node itself, exactly as in Unix, where an inode contains no name. Three node types are defined. A \textsc{file} node holds a content hash, MIME type, and file size. A \textsc{dir} node holds a children list and a counter for the next available child index. A \textsc{link} node holds a target reference in one of three forms: \textsc{hard} (pointing to a specific transaction for a fixed version), \textsc{soft} (pointing to a $P_{\text{node}}$, always resolving to the latest version), or \textsc{soft\_remote} (pointing to a domain and path for cross-user references resolved via DNS).

Hard links behave as true Unix hard links: multiple directory entries in possibly different directories reference the same $P_{\text{node}}$, sharing the underlying file data. Soft links introduce an indirection layer, always resolving to the latest version of the target. Remote soft links extend this across trust boundaries, referencing a domain and path that resolve through DNS, enabling cross-user file references without requiring both parties to share a Metanet tree.
...
\item $\text{aes\_key} = \text{HKDF-SHA256}(\text{ikm}=\text{shared\_point}.x,\; \text{salt}=\text{key\_hash})$
...
For directory-level purchases, BIP32 key derivation provides a natural solution. Because ECDH is algebraically compatible with the non-hardened derivation of BIP32, a single capsule for a parent directory can unlock all non-hardened child nodes:
...
\textbf{DNS binding} associates any Metanet node with a domain name through two DNS records: a TXT record at \texttt{\_bitfs\_pubkey} containing the public key of the node, and one or more SRV records at \texttt{\_bitfs.\_tcp} specifying service endpoints with priority and weight for load balancing. Bidirectional verification ensures consistency: the DNS record points to $P_{\text{node}}$, and the domain field of the Metanet payload records the bound domain. Both must agree.
...
BitFS defines the file system protocol: how files are organized, encrypted, addressed, and traded. Content distribution---the question of how encrypted bytes travel from storage to the requesting client---is handled by the Metanet network, a decentralized CDN described in its own whitepaper~\cite{metanet_multi}.
...
UK Intellectual Property Office, 2025.
